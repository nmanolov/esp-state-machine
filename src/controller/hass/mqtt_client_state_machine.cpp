...
ClientContext::ClientContext(AsyncMqttClient &pAsyncMqttClient,
    ...
    : mqttClient(pAsyncMqttClient), integrations(pIntegrations),
...
unsigned int ClientContext::getTimeout() const { return nextConnectionWait; }

ClientState *setupClient(ClientState *self,
                         StateMachineHandle<ClientContext> &machine,
                         const ClientSygnals &sygnal) {
...
ClientState *connecting(ClientState *self,
                        StateMachineHandle<ClientContext> &machine,
                        const ClientSygnals &sygnal) {
...
ClientState *arbitraryDelay(ClientState *self,
                            StateMachineHandle<ClientContext> &machine,
                            const ClientSygnals &sygnal) {
...
ClientState *working(ClientState *self,
                     StateMachineHandle<ClientContext> &machine,
                     const ClientSygnals &sygnal) {
...
ClientSM::ClientSM(ClientContext &pContext)
    : NStateMachine<ClientSygnals, ClientContext>(pContext, &setupClient,
...
void ClientSM::setUp() {
  ...
  for (auto integration : context.integrations) {
    if(integration->hasWill()) {
      mqttClient.setWill(integration->willTopic(), 0, true, integration->willPayload());
      break;
    }
  }
}
